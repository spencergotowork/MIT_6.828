...
sys_env_set_status(envid_t envid, int status)
{
	...
	int ret = envid2env(envid, &realenv, 1);
	if(ret < 0) return ret;

	realenv->env_status = status;
	return 0;
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	if(!tf)
		panic("sys_env_set_trapframe is wrong! Trapframe\n");
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if(srcva >= (void*)UTOP || dstva >= (void*)UTOP ||ROUNDDOWN(srcva,PGSIZE) != srcva 
...
{
	// if((uintptr_t)dstva > UTOP) return -1;
	// LAB 4: Your code here.
